...
export function PropertyRiskReportModal({
  property, open, onClose, compareProperty = null,
  ...
  const fetchCompare = useCallback(() => {
    ...
    getPropertyReport(compareProperty)
      ...
      .catch((err: unknown) => compareDispatch({
        type: 'ERROR',
  ...
  useEffect(() => { if (showCompare && compareProperty) fetchCompare() }, [showCompare, compareProperty, fetchCompare])
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen: boolean) => !isOpen && onClose()}>
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-hidden p-0 gap-0 flex flex-col" aria-label="Property risk report">
        <div className="sticky top-0 z-10 bg-white border-b border-gray-200">
          <div className="px-6 py-4 flex items-start justify-between gap-4">
            ...
            {state.status === 'success' && (
              <ReportActions property={property} fullAddress={fullAddress} onPrint={handlePrint} onCompare={() => setShowCompare((v) => !v)} showCompare={showCompare} />
            )}
          </div>
          {state.status === 'success' && (
            <div className="px-6 flex gap-1" role="tablist" aria-label="Report sections">
              <button role="tab" aria-selected={activeTab === 'risks'} onClick={() => setActiveTab('risks')}
              ...
              <button role="tab" aria-selected={activeTab === 'carriers'} onClick={() => setActiveTab('carriers')}
        ...
        <div ref={contentRef} className="flex-1 overflow-y-auto" id="report-print-area">
          <div className={`flex ${showCompare ? 'divide-x divide-gray-200' : ''}`}>
            <div className={`${showCompare ? 'w-1/2' : 'w-full'} px-6 py-6`}>
              ...
              {state.status === 'error' && (
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <button onClick={fetchReport} className="mt-4 px-4 py-2 text-sm font-semibold bg-gray-900 hover:bg-gray-800 text-white rounded-lg transition">
                    Retry
                  </button>
                </div>
              )}
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                          {RISK_CATEGORIES.map((cat) => (
                            <RiskCostCard key={cat} category={cat} meta={RISK_META[cat]} riskProfile={state.data.risk} costEstimate={state.data.insurance} />
                          ))}
                        </div>
                      </div>
            ...
            {showCompare && (
              <CompareDrawer compareProperty={compareProperty} compareState={compareState} onRetry={fetchCompare} activeTab={activeTab} riskMeta={RISK_META} riskCategories={RISK_CATEGORIES} />
            )}
          </div>
        </div>
      </DialogContent>
