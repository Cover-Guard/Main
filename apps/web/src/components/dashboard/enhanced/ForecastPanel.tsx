...
interface SeriesToggleProps { label: string; color: string; active: boolean; onClick: () => void; }

function SeriesToggle({ label, color, active, onClick }: SeriesToggleProps) {
  return (
    <button onClick={onClick}
      className={`flex items-center gap-1 px-1.5 py-0.5 rounded text-xs font-medium transition-colors ${active ? 'bg-white border border-gray-200 shadow-sm' : 'bg-gray-100 text-gray-400 line-through'}`}>
...
export function ForecastPanel() {
  ...
  // PR-B1.f: pull 12-month forecast. Replaces FORECAST_DATA / FORECAST_EXTENDED mocks.
  // eslint-disable-next-line react-hooks/set-state-in-effect -- B1.f2 will refactor
  useEffect(() => { getDashboardForecast().then(setResponse).catch(() => setResponse(null)); }, []);

  const fullSeries: ForecastDataPoint[] = response?.series ?? [];
  const data = range === '10mo' ? fullSeries : fullSeries.slice(-6);

  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-1.5"><LiveDot /><span className="text-xs text-gray-500">Rolling forecast</span></div>
        <div className="flex items-center gap-1">
          ...
          <button onClick={() => setRange('10mo')} className={`px-1.5 py-0.5 rounded text-xs font-medium ${range === '10mo' ? 'bg-indigo-100 text-indigo-700' : 'text-gray-400 hover:text-gray-600'}`}>10mo</button>
          <span className="w-px h-3 bg-gray-200 mx-0.5" />
          <button onClick={() => setShowTable(!showTable)} className={`px-1.5 py-0.5 rounded text-xs font-medium ${showTable ? 'bg-indigo-100 text-indigo-700' : 'text-gray-400 hover:text-gray-600'}`}>Table</button>
        </div>
      </div>
      ...
      {!showTable ? (
        <ResponsiveContainer width="100%" height={160}>
          <AreaChart data={data}>
            <defs>
              <linearGradient id="colorPrem" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#6366f1" stopOpacity={0.15} /><stop offset="95%" stopColor="#6366f1" stopOpacity={0} /></linearGradient>
              <linearGradient id="colorProj" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#a78bfa" stopOpacity={0.1} /><stop offset="95%" stopColor="#a78bfa" stopOpacity={0} /></linearGradient>
            </defs>
          ...
          <table className="w-full text-xs">
            <thead className="sticky top-0 bg-white">
              <tr className="border-b">
                {['Month', 'Premium', 'Projected', 'Claims', 'Loss %'].map((h) => <th key={h} className="text-left py-1 px-2 text-gray-500 font-medium">{h}</th>)}
              </tr>
            </thead>
            <tbody>
              {data.map((d) => (
