      ...
      <div className="hidden w-1/2 flex-col justify-between bg-brand-900 p-12 lg:flex">
        ...
        <div>
          ...
          <h1 className="text-4xl font-bold leading-tight text-white">
            Know insurability<br />before every bid.
          </h1>
          <p className="mt-4 text-lg text-brand-300">
            Help your clients make confident offers by surfacing flood, fire, and carrier availability data for any US property — in seconds.
          </p>
          <div className="mt-8 space-y-3">
            {[
              'Instant risk scores from FEMA, USGS & Cal Fire',
              'Live carrier writing status by property',
              'Request binding quotes in one click',
              'Manage clients and compare properties',
            ].map((item) => (
        ...
        <p className="text-xs text-brand-500">© {new Date().getFullYear()} CoverGuard. All rights reserved.</p>
      </div>
        ...
        <div className="w-full max-w-sm">
          ...
          <div className="mb-6">
            <div className="mb-3 flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-brand-700">Agent / Team Portal</span>
            </div>
            <h1 className="text-2xl font-bold text-gray-900">Sign in</h1>
            <p className="mt-1 text-sm text-gray-500">
              Not an agent?{' '}
              <Link href="/login" className="text-brand-600 hover:underline">Consumer login →</Link>
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <button type="submit" disabled={isSubmitting || oauthLoading} className="btn-primary w-full py-2.5">
              {isSubmitting ? 'Signing in…' : 'Sign in to Agent Portal'}
            </button>
          </form>

          <p className="mt-4 text-center text-sm text-gray-500">
            New to CoverGuard?{' '}
            <Link href="/agents/register" className="text-brand-600 hover:underline">Create agent account</Link>
          </p>
        </div>
      </div>
    </div>
